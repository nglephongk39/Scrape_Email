...
for index in range(len(List_Company_Name_Old)):
    Company_Name_New = str(List_Company_Name_Old[index]).replace(' ', "+")
    # Here we instantiate the webdriver, so that we can use it for the project.
    #Hide the website when run code.
    ...
    driver.get(f"https://www.google.com/search?q={Company_Name_New}+*email")
    sleep(4)
    ...
    try:
        ...
        sh1[f'AD{index + start}'].value = str(list_of_emails[0])
    except:
        sh1[f'AD{index + start}'].value = ''
